...
h2o_gradient = [
    [0.0000033590, -0.0000021343, -0.0331173999],
...
def run_py_test(port=54321, run_real_server=False):
    ...
    with TCProtobufClient(host="localhost", port=port, trace=run_real_server) as TC:
        ...
        if not np.allclose([h2o_energy], [energy], atol=tol) or not np.allclose(
            h2o_gradient, gradient, atol=tol
        ):
...
def run_cpp_test(port=56789):
    ...
    # Set up MockServer for testing
    # mock = MockServer(
    #     port, "energy_grad_force/client_recv.bin", "energy_grad_force/client_sent.bin"
    # )

    # Subprocess out, expect a returncode of 1 for failure and 0 for success
    rc = subprocess.call(
        "./energy_grad_force/energy_grad_force_test localhost {}".format(port),
        shell=True,
    )
